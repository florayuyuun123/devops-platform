...
import subprocess, json, time, secrets
...
# Dynamic path — works locally and in Docker
...
def health():
    return {
        "status": "ok",
        "platform": "DevOps Learning Platform",
        "version": "1.0.0",
        "labs_path": LABS_PATH,
        "labs_found": os.path.exists(LABS_PATH)
    }
...
def get_labs():
    ...
    if os.path.exists(LABS_PATH):
        for phase_dir in sorted(os.listdir(LABS_PATH)):
            meta_file = os.path.join(LABS_PATH, phase_dir, "meta.json")
        ...
        for phase_dir in sorted(os.listdir(LABS_PATH)):
            ...
            if os.path.exists(meta_file):
                ...
                if meta.get("id") == lab_id:
                    lab_file = os.path.join(LABS_PATH, phase_dir, "LAB.md")
...
def start_sandbox(req: SandboxRequest):
    ...
    lab_path = os.path.join(LABS_PATH, req.lab_id)
    cmd = [
        "docker", "run", "-d",
        "--name", container_name,
        "--memory", "512m",
